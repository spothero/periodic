...
// AddDSTAware will add the given duration to the given time, adjusting for timezone offset changes due to DST and return
...
func AddDSTAware(t time.Time, d time.Duration) time.Time {
	result := t.Add(d)
	_, tOffset := t.Zone()
